...
def create_contact(contact: ContactCreate, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.create_contact(contact)

@router.get("/contacts/{contact_id}", response_model=ContactResponse)
def get_contact(contact_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.get_contact(contact_id)

@router.get("/workspaces/{workspace_id}/contacts", response_model=List[ContactResponse])
def get_workspace_contacts(workspace_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.get_workspace_contacts(workspace_id)

@router.put("/contacts/{contact_id}", response_model=ContactResponse)
def update_contact(contact_id: UUID, contact: ContactUpdate, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.update_contact(contact_id, contact)

@router.delete("/contacts/{contact_id}")
def delete_contact(contact_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.delete_contact(contact_id)

# Deal routes
@router.post("/deals", response_model=DealResponse)
def create_deal(deal: DealCreate, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.create_deal(deal)

@router.get("/deals/{deal_id}", response_model=DealResponse)
def get_deal(deal_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
...
def get_contact_deals(contact_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.get_contact_deals(contact_id)

@router.put("/deals/{deal_id}", response_model=DealResponse)
def update_deal(deal_id: UUID, deal: DealUpdate, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
    return handler.update_deal(deal_id, deal)

@router.delete("/deals/{deal_id}")
def delete_deal(deal_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
...
def get_contact_activities(contact_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
...
def get_deal_activities(deal_id: UUID, user: dict = Depends(get_auth_details)):
    handler = CRMHandler()
